...
| Integrazioni CRM | **Fondazione rilasciata:** configurazione HubSpot/Pipedrive per tenant senza segreti nel database, invio esplicito dei lead, retry idempotente, stato visibile nel panel e Worker Cloudflare con upsert HubSpot/Pipedrive per email. Restano OAuth self-service, mapping campi e sincronizzazione automatica asincrona; poi Salesforce in base alla domanda. | Alta |
