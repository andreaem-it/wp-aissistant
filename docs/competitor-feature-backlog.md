...
| API pubblica e webhooks gestibili | **Rilasciata:** API `/v1` versionata, chiavi scoped e revocabili con rate limit dedicato, webhook firmati HMAC con retry a backoff, payload con schema `1.0`, eventi omnicanale privacy-safe su ogni messaggio visitatore, log consegne con ispezione JSON nel panel, replay manuale senza sovrascrivere lo storico e guardia SSRF. Documentazione in [`public-api.md`](public-api.md). | Prossimo passo: filtri e ricerca nello storico consegne. | Media |
