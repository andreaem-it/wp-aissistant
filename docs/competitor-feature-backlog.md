...
| Monitoraggio e alerting operativo | Implementato; **verificato in produzione il 14 agosto 2026**: `/health` risponde `200`, `/docs` e `/metrics` sono spenti (`404`, quest'ultimo perché `METRICS_TOKEN` non è impostata — quindi lo scraping Prometheus non è attivo, ed è una scelta da confermare, non un difetto). Restano da verificare i destinatari degli alert | Alto | Media | Sentry, uptime, metriche e regole alert per down/5xx/latenza/ingest/provider AI con runbook. |
...
| Hardening produzione | **Verificato in produzione il 14 agosto 2026**, e non a occhio: `production_config.production_warnings()` eseguito contro le 51 variabili vere del servizio non produce nessun avviso, con `STRICT_PRODUCTION_CONFIG=true` — quindi il backend si rifiuterebbe di partire se la configurazione peggiorasse. Il pregio del metodo è che il controllo è **il nostro**: una regola aggiunta domani viene applicata da questa stessa verifica | Alto | Bassa | Validazione fail-closed con `STRICT_PRODUCTION_CONFIG`, warning in admin health e stack production sicuri. |
...
| Assistente dentro il panel | **Rilasciato.** Il widget vero (importato dal workspace, non dal CDN: una pagina autenticata non deve perdere l'assistenza se il CDN ha un problema) con la nostra knowledge base. Il contesto del tenant loggato **non arriva dal browser** — la `api_key` del widget è pubblica per costruzione — ma da un token HMAC di 5 minuti emesso dietro sessione operatore, e i fatti li rilegge il backend dal database. Il contesto è una whitelist di dieci campi verificata *per uguaglianza* da un test, perché un campo nuovo nel prompt di un modello è una decisione. Onorato solo se a rispondere è un tenant di piattaforma: un token è legato a chi l'ha chiesto e non legge dati altrui, ma presentato al widget di un altro cliente riverserebbe i propri dati nella casella di quello. Vedi [roadmap](embedded-assistant-roadmap.md). | Media |
