...
|---|---|---:|---:|---|
| GDPR e residenza dei dati UE | **Audit completato, remediation P0 in corso** | Alto | Alta | Il 9 agosto backend Railway è stato spostato da `sfo` a EU West e gli allegati da un semplice hint `EEUR` a R2 jurisdiction `eu`. Restano bloccanti: Workers AI senza garanzia di elaborazione solo UE, retention da approvare e attivare, Redis necessario per strict mode, DPA/SCC e registro subprocessori, DPIA e pagine legali. Non usare ancora claim «tutti i dati in UE» o «100% GDPR compliant». Piano ed evidenze in [`gdpr-eu-residency-audit.md`](gdpr-eu-residency-audit.md). |
