...
| Visibilità commerciale (superadmin) | Implementata la vista ricavi; costi/margine e azioni commerciali restano da fare | Alto | Media | `GET /admin/revenue` e vista **Ricavi** nel panel admin: MRR/ARR normalizzati sull'intervallo reale di fatturazione, ricavo medio, ripartizione per piano, insoluti, prove in scadenza, disdette programmate e del periodo. Incassato, a rischio e in prova restano separati; le disdette sono un conteggio, non un tasso inventato. Prossimi passi: costo e margine per tenant da `AiResponseLog`, azioni commerciali via API Stripe (trial, coupon, sospensione) e funnel di attivazione. |
