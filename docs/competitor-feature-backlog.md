...
|---|---|---:|---:|---|
| GDPR e residenza dei dati UE | **Audit completato, remediation P0 aperta** | Alto | Alta | La verifica live del 9 agosto 2026 ha trovato backend Railway in `sfo`, Workers AI senza garanzia di elaborazione solo UE, R2 con location hint `EEUR` non vincolante, retention disattivata e strict mode non attiva. Prima di usare claim forti: backend in EU West, inferenza EU-only contrattuale, R2 jurisdiction `eu`, retention/strict attive, DPA/SCC e registro subprocessori, DPIA e pagine legali pubblicate. Piano ed evidenze in [`gdpr-eu-residency-audit.md`](gdpr-eu-residency-audit.md). |
