...
| Visibilità commerciale (superadmin) | Implementate ricavi, costi, margine e azioni commerciali | Alto | Media | Viste **Ricavi** e **Costi e margine** nel panel admin. Ricavi: MRR/ARR normalizzati sull'intervallo reale di fatturazione, ripartizione per piano, insoluti, prove in scadenza, disdette programmate e del periodo; incassato, a rischio e in prova restano separati e le disdette sono un conteggio, non un tasso inventato. Costi: spesa AI per tenant dai token di `AiResponseLog`, con listino per modello inserito nella cifra pubblicata dal provider (decimali conservati), margine contro il ricavo ricorrente, modelli senza prezzo esclusi dai totali e segnalati, e valute diverse dichiarate invece che convertite a un tasso arbitrario. Solo inferenza: embedding, storage e canali non sono ancora contati. Azioni commerciali (proroga prova, coupon, sospensione, disdetta, cambio piano) eseguite **su Stripe** con il webhook come unico scrittore dello stato, quindi niente disallineamento fra database e fatturazione. Funnel di attivazione (creato → plugin → prima chat → prima risposta utile → primo pagamento) con tempo mediano, elenco dei bloccati e coorti senza data dichiarate a parte; clienti a rischio con motivi espliciti invece di un punteggio. Prossimo passo: costi di embedding e storage per chiudere il margine. |
