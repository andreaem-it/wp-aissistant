...
| API pubblica e webhooks gestibili | **Rilasciata:** API `/v1` versionata, chiavi scoped e revocabili con rate limit dedicato, webhook firmati HMAC con retry a backoff, log consegne nel panel, replay manuale delle consegne fallite senza sovrascrivere lo storico e guardia SSRF. Documentazione in [`public-api.md`](public-api.md). | Prossimo passo: eventi su ogni messaggio del visitatore. | Media |
