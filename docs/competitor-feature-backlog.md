...
| API pubblica e webhooks gestibili | **Rilasciata:** API `/v1` versionata, chiavi scoped e revocabili con rate limit dedicato, webhook firmati HMAC con retry a backoff, eventi omnicanale privacy-safe su ogni messaggio visitatore, log consegne nel panel, replay manuale delle consegne fallite senza sovrascrivere lo storico e guardia SSRF. Documentazione in [`public-api.md`](public-api.md). | Prossimo passo: schema JSON versionato e pagina di ispezione del payload. | Media |
...
| Instagram/Facebook Messenger | **Fondazione rilasciata nel backend:** adapter inbound condiviso e provider-neutral, deduplicazione, threading, contatto unificato, ticket/SLA e risposta operatore tramite adapter tenant-aware. Restano Worker Meta live, credenziali, allegati e verifica end-to-end. | Alta |
...
| Integrazioni help desk | **Fondazione rilasciata:** connessioni Zendesk/Freshdesk tenant-scoped, handoff manuale idempotente con contatto e transcript completo, stato e retry nel panel, credenziali isolate in un adapter esterno. Restano configurazione guidata delle credenziali provider, sincronizzazione bidirezionale e Intercom. | Alta |
...
| SDK/widget headless | **Fondazione rilasciata:** client browser framework-agnostic con sessione persistente, chat sincrona e streaming SSE, polling, feedback, contatto, ticket, CSAT e lead form; Web Component accessibile e isolato in Shadow DOM; token conversazione mai nella URL e HTTPS obbligatorio. Resta la pubblicazione del pacchetto npm. | Alta |
