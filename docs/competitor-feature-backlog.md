...
| API pubblica e webhooks gestibili | **Rilasciata:** API `/v1` versionata, chiavi scoped e revocabili con rate limit dedicato, webhook firmati HMAC con retry a backoff, payload con schema `1.0`, eventi omnicanale privacy-safe, log consegne filtrabile con ispezione JSON, replay manuale e guardia SSRF. Documentazione in [`public-api.md`](public-api.md). | Prossimo passo: paginazione cursor-based dello storico. | Media |
