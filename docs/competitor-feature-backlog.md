...
| API pubblica e webhooks gestibili | **Rilasciata:** API `/v1` versionata, chiavi scoped e revocabili con rate limit dedicato, webhook firmati HMAC con retry a backoff, payload con schema `1.0`, eventi omnicanale privacy-safe, log filtrabile/paginato con JSON, replay manuale, metriche di affidabilità a 30 giorni e guardia SSRF. Documentazione in [`public-api.md`](public-api.md). | Prossimo passo: alert automatico su endpoint degradato. | Media |
