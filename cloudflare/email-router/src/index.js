...
export default {
  async email(message, env) {
    ...
    if (!text) return message.setReject("550 Corpo testuale mancante");
    ...
    const response = await fetch(`${env.BACKEND_URL.replace(/\/$/, "")}/channels/email/inbound`, {
      ...
      body: JSON.stringify({from_email:message.from,from_name:parsed.from?.name||"",subject:parsed.subject||"",text,message_id:messageId,thread_id:rootThreadId(messageId,inReplyTo,references),in_reply_to:inReplyTo}),
    });
